...
/***/ function(module, exports, __webpack_require__) {
	...
	var init = function init() {
		FN.Plugin.register({
			'job-select0': __webpack_require__(155).default,
			'job-select1': __webpack_require__(155).default
		});
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var JobSelect = function (_FN$Widget) {
		(0, _inherits3.default)(JobSelect, _FN$Widget);
	
		function JobSelect() {
			(0, _classCallCheck3.default)(this, JobSelect);
			return (0, _possibleConstructorReturn3.default)(this, (JobSelect.__proto__ || (0, _getPrototypeOf2.default)(JobSelect)).apply(this, arguments));
		}
	
		(0, _createClass3.default)(JobSelect, [{
			key: 'initAttrs',
			value: function initAttrs() {
				var attrs = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	
	
				attrs = (0, _get3.default)(JobSelect.prototype.__proto__ || (0, _getPrototypeOf2.default)(JobSelect.prototype), 'initAttrs', this).call(this, attrs) || {};
				...
				return (0, _extends3.default)({
					...
					roleTriggerText: 'span',
					roleMask: '[dropdown-role="mask"]',
					rolePanel: '[dropdown-role="panel"]',
					attrValue: 'dropdown-value',
					attrText: 'dropdown-text'
				}, attrs);
			}
		}, {
			key: '_setValue',
			value: function _setValue(value) {
				...
				var $option = this.$(this.get('rolePanel')).children('[' + this.get('attrValue') + '="' + value + '"]');
				if ($option.length <= 0) return;
	
				var selectedClassName = this.get('selectedClassName');
				$option.addClass(selectedClassName).siblings().removeClass(selectedClassName);
	
				if (this.get('roleTriggerText')) {
					$(this.get('trigger')).find(this.get('roleTriggerText')).text($option.attr(this.get('attrText')));
				}
			}
		}, {
			key: 'render',
			value: function render() {
				this._setValue(this.get('initialValue'));
			}
		}, {
			key: 'initEvents',
			value: function initEvents() {
				...
				this.$(this.get('rolePanel')).children().on('click', function (event) {
					_this2.handleSelect($(event.currentTarget));
				});
			}
		}, {
			key: 'handleSelect',
			value: function handleSelect($el) {
				var value = $el.attr(this.get('attrValue'));
			...
			value: function show() {
				...
				$('body').addClass('ovh');
				this.el.addClass(this.get('activeClassName'));
	
				// const ns = `click.${this.get('id')}`;
	
				// setTimeout(() => {
				// 	$(document).off(ns).on(ns, () => {
				// 		this.hide();
				// 		$(document).off(ns);
				// 	});
				// }, 30);
			...
			value: function hide() {
				...
				this.el.removeClass(this.get('activeClassName'));
			}
		}]);
		return JobSelect;
	}(FN.Widget);
	
	exports.default = JobSelect;

/***/ }
/******/ ]);
//# sourceMappingURL=p3-social-recruitment.js.map