...
/***/ function(module, exports, __webpack_require__) {
	...
	var init = function init() {
		FN.Plugin.register({
			RedPacketRainPlugin: __webpack_require__(118).default
		});
	};
	
	if (typeof window.define === 'function' && window.define.amd) {
		window.define(['fnModule/common/vendor', 'fnLib/hilo/hilo'], init);
	} else {
		init();
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var RedPacketRainPlugin = function (_FN$Widget) {
		(0, _inherits3.default)(RedPacketRainPlugin, _FN$Widget);
	
		function RedPacketRainPlugin() {
			(0, _classCallCheck3.default)(this, RedPacketRainPlugin);
			return (0, _possibleConstructorReturn3.default)(this, (RedPacketRainPlugin.__proto__ || (0, _getPrototypeOf2.default)(RedPacketRainPlugin)).apply(this, arguments));
		}
	
		(0, _createClass3.default)(RedPacketRainPlugin, [{
			key: 'initAttrs',
			value: function initAttrs(attrs) {
				attrs = (0, _get3.default)(RedPacketRainPlugin.prototype.__proto__ || (0, _getPrototypeOf2.default)(RedPacketRainPlugin.prototype), 'initAttrs', this).call(this, attrs) || {};
				...
				return (0, _extends3.default)({
					...
					startTipCountdown: 3,
					roleDialog: '[dialog]',
					roleDialogClose: '[dialog-role="close"]',
					roleStageEnter: '[stage="enter"]',
					roleStageCanvas: '[stage="canvas"]',
					roleStageEnd: '[stage="end"]',
					roleStageDialog: '[stage="dialog"]'
				}, attrs);
			}
		}, {
			key: 'load',
			value: function load() {
				var resources = [{ id: 'redPacket', src: this.get('redPacketPath') }, { id: 'crazyGuy', src: this.get('crazyGuyPath') }, { id: 'clickedPacket', src: this.get('clickedPacketPath') }];
	
				var queue = new Hilo.LoadQueue(resources);
			...
			value: function render() {
				...
				var role = this.role = {
					stageEnter: this.$(this.get('roleStageEnter')),
					stageCanvas: this.$(this.get('roleStageCanvas')),
					stageEnd: this.$(this.get('roleStageEnd')),
					stageDialog: this.$(this.get('roleStageDialog'))
				};
	
				var loadStatus = this.load();
				var stageCreated = false;
				var createStage = function createStage() {
					stageCreated = true;
	
					loadStatus.then(function (queue) {
						role.stageEnter.hide();
						role.stageCanvas.show();
	
						_this2.queue = queue;
						FN.Radio.fire('countdownStart');
						_this2.initStage();
					});
				};
				...
				setTimeout(function () {
					...
					createStage();
				}, this.get('startTipCountdown') * 1000);
	
				role.stageEnter.on('click', function () {
					if (stageCreated) return;
					createStage();
				});
	
				role.stageDialog.on('click', this.get('roleDialogClose'), function () {
					role.stageDialog.hide();
					_this2.trigger('dialogClose');
					_this2.resumeRain();
				});
			}
		}, {
			...
			value: function initStage() {
				...
				var $stageCanvas = this.role.stageCanvas;
	
				var stageWidth = this.stageWidth = $stageCanvas.width();
				var stageHeight = this.stageHeight = $stageCanvas.height();
				var width = this.width = 750;
				var height = this.height = 750 * (stageHeight / stageWidth);
				...
				$stageCanvas.prepend(this.stage.canvas);
	
				// 阻止事件
				$(this.stage.canvas).on('touchstart', function (event) {
				...
				// 创建红包动画
				this.addRedPacket(0, height * 0.6, 10);
				this.addRedPacket(0, height * 0.4, 10);
				this.addRedPacket(0, height * 0.2, 14, 3);
				this.addRedPacket(0, height * 0, 14, 3, 400);
				this.addRedPacket(width * 0.4, 0, 10);
				this.addRedPacket(width * 0.8, 0, 10);
	
				// 底部小人
				var crazyGuyHeight = this.get('crazyGuyHeight');
				new Hilo.Bitmap({
					image: this.queue.get('crazyGuy').content,
					rect: [0, 0, 750, crazyGuyHeight],
					x: 0,
					y: height - crazyGuyHeight
				}).addTo(stage);
	
				this._rainStatus = 'running';
	
				var countdown = this.get('countdown') - 1;
				var $cd = $stageCanvas.find('[data-role="countdown"]');
				var _exc = function _exc() {
					// 如果下雨的状态是暂停
					if (_this3._rainStatus !== 'running') {
						return;
					}
					...
					if (countdown < 0) {
						_this3.stopRain();
						FN.Radio.trigger('countdownComplete');
						/* eslint-disable */
						iter && window.clearInterval(iter);
						/* eslint-enable */
					}
				};
				var iter = setInterval(_exc, 1000);
	
				_exc();
			}
		}, {
			key: 'pauseRain',
			value: function pauseRain() {
				if (this._rainStatus !== 'running') return;
				this._rainStatus = 'paused';
	
				this.ticker.pause();
			}
		}, {
			key: 'resumeRain',
			value: function resumeRain() {
				if (this._rainStatus !== 'paused') return;
				this._rainStatus = 'running';
			...
			key: 'stopRain',
			value: function stopRain() {
				if (this._rainStatus === 'stop') return;
	
				this._rainStatus = 'stop';
				this.ticker.stop();
			}
		}, {
			key: 'toEndsTip',
			value: function toEndsTip() {
				this.stopRain();
				this.role.stageEnd.show();
			}
		}, {
			key: 'showDialog',
			value: function showDialog(type) {
				var stageDialog = this.role.stageDialog;
	
	
				this.pauseRain();
				stageDialog.find('[dialog]').hide();
				stageDialog.show();
				stageDialog.find('[dialog="' + type + '"]').show();
			}
		}, {
			key: 'addRedPacket',
			value: function addRedPacket(x1, y1) {
				...
				var img = this.queue.get('redPacket').content;
				...
				if (y1 === 0) {
					...
					x2 = this.height - y1 + 80;
					y2 = this.height + 80;
				}
	
				var add = function add() {
					// create a bitmap
					var redPacket = new Hilo.Bitmap({
						image: img,
						rect: [0, 0, 160, 160],
						height: 160,
						width: 160,
					...
					redPacket.on(Hilo.event.POINTER_START, function () {
						// 如果已经停止，则什么也不做
						if (_this4._rainStoped) return;
	
						redPacket.__isOpend = true;
						_this4.openRedPacket(redPacket.x, redPacket.y);
						// this.openRedPacket(e.stageX, e.stageY);
					});
				};
			...
			key: 'openRedPacket',
			value: function openRedPacket(x, y) {
				var clickedPacket = new Hilo.Bitmap({
					image: this.queue.get('clickedPacket').content,
					rect: [0, 0, 160, 160],
					height: 160,
					width: 160,
					x: x,
				...
				setTimeout(function () {
					clickedPacket.removeFromParent();
				}, 300);
	
				if (Math.random() >= 1 - this.get('probability')) {
					FN.Radio.trigger('redPacketOpen');
				}
			}
		}]);
		return RedPacketRainPlugin;
	}(FN.Widget);
	
	exports.default = RedPacketRainPlugin;
...
/***/ }
/******/ ]);
//# sourceMappingURL=red-packet-rain.js.map